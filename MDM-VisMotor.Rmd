...
```
## Sum of LPLs for each session
```{r Sum of LPLs for each session, fig.height=7, fig.width=6}
lpl1 = apply(rmna(mdm.s1$lpl), c(1,2), sum)
lpl2 = apply(rmna(mdm.s2$lpl), c(1,2), sum)
lpl3 = apply(rmna(mdm.s3$lpl), c(1,2), sum)
lpl4 = apply(rmna(mdm.s4$lpl), c(1,2), sum)
lpl5 = apply(rmna(mdm.s5$lpl), c(1,2), sum)

p1 = gplotMat(lpl1, title = "rest", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl1), max(lpl1[lpl1 < 0]) ))
p2 = gplotMat(lpl2, title = "motor", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl2), max(lpl2[lpl2 < 0]) ))
p3 = gplotMat(lpl3, title = "visual", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl3), max(lpl3[lpl3 < 0]) ))
p4 = gplotMat(lpl4, title = "motor/visual", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl4), max(lpl4[lpl4 < 0]) ))
p5 = gplotMat(lpl5, title = "motor/visual II", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl5), max(lpl5[lpl5 < 0]) ))
...
```{r Common network across subjects and sessions, fig.height=5, fig.width=6}
...
# sum across runs
lpl = rmna(mdm.s1$lpl) + rmna(mdm.s2$lpl) + rmna(mdm.s3$lpl) + rmna(mdm.s4$lpl) + rmna(mdm.s5$lpl)
# sum across subjects
lpl = apply(lpl, c(1,2), sum)

p1 = gplotMat(stats$adj, title = "Common net", nodeLabels = mylabels, axisTextSize=8, xAngle=90)
p2 = gplotMat(rmna(stats$adj_fdr), title = "binom test & FDR",
              nodeLabels = mylabels, axisTextSize=8, xAngle=90)

p3 = gplotMat(tstats$adj, title = "thresholded", nodeLabels = mylabels, axisTextSize=8, xAngle=90)
p4 = gplotMat(rmna(tstats$adj_fdr), title = "binom test & FDR",
              nodeLabels = mylabels, axisTextSize=8, xAngle=90)

p5 = gplotMat(lpl, title = "Common net sum(lpl)", nodeLabels = mylabels, axisTextSize=8, xAngle=90,
              lim = c(min(lpl), max(lpl[lpl < 0]) ))


plot_grid(p1, p2, p3, p4, p5, ncol = 2, nrow = 2, rel_heights = c(1,1,1))
```
...
s = 5
df = mean(c(mdm.s1$df_[,node], mdm.s2$df_[,node], mdm.s3$df_[,node], mdm.s4$df_[,node], mdm.s5$df_[,node]))

# select time series of parents from best model
#pars=mdm.s1$winner[2:Nn,node,s]
#pars=pars[pars!=0]

# time x parents x session x subjects
theta = array(NA, dim=c(Nt, length(pars)+1, Ns, N))
...
```{r Plot connectivity strength Occ pole, fig.height=8, fig.width=10}
lim=c(-1.5, 1.5)

p1 = ggplot(melt(theta[,2,1,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("rest") + ylim(lim)

p2 = ggplot(melt(theta[,2,2,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("motor") + ylim(lim)

p3 = ggplot(melt(theta[,2,3,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("visual") + ylim(lim)

p4 = ggplot(melt(theta[,2,4,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("visual/motor") + ylim(lim)

p5 = ggplot(melt(theta[,2,5,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
...
```{r Plot connectivity strength inf lat occ, fig.height=8, fig.width=10}
lim=c(-1.5, 1.5)

p1 = ggplot(melt(theta[,3,1,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("rest") + ylim(lim)

p2 = ggplot(melt(theta[,3,2,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("motor") + ylim(lim)

p3 = ggplot(melt(theta[,3,3,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("visual") + ylim(lim)

p4 = ggplot(melt(theta[,3,4,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("visual/motor") + ylim(lim)

p5 = ggplot(melt(theta[,3,5,]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) + geom_line() +
  theme_minimal() + ggtitle("visual/motor II") + ylim(lim)
...
```{r Mean/Variance of the theta_t across runs, fig.height=2, fig.width=6}

#dims:  230   4   5  15
M=array(NA, dim=c(N, Ns, length(pars)))
M[,1,1] = apply(theta[,2,1,], 2, mean)
M[,2,1] = apply(theta[,2,2,], 2, mean)
M[,3,1] = apply(theta[,2,3,], 2, mean)
M[,4,1] = apply(theta[,2,4,], 2, mean)
M[,5,1] = apply(theta[,2,5,], 2, mean)

M[,1,2] = apply(theta[,3,1,], 2, mean)
M[,2,2] = apply(theta[,3,2,], 2, mean)
M[,3,2] = apply(theta[,3,3,], 2, mean)
M[,4,2] = apply(theta[,3,4,], 2, mean)
M[,5,2] = apply(theta[,3,5,], 2, mean)

M[,1,3] = apply(theta[,4,1,], 2, mean)
M[,2,3] = apply(theta[,4,2,], 2, mean)
M[,3,3] = apply(theta[,4,3,], 2, mean)
M[,4,3] = apply(theta[,4,4,], 2, mean)
M[,5,3] = apply(theta[,4,5,], 2, mean)

p1 = ggplot(melt(M[,,1]), aes(x=as.factor(Var2), y=value)) + geom_violin() + 
  geom_boxplot(width=0.2) + ggtitle("Occ Pole") + ylab("mean theta_t") + xlab("session") +
  geom_point(shape=1, color="gray50", size=2,
             position = position_jitter(width = 0.2, height = 0.0002))

p2 = ggplot(melt(M[,,2]), aes(x=as.factor(Var2), y=value)) + geom_violin() + 
  geom_boxplot(width=0.2) + ggtitle("inf lat occ") + ylab("mean theta_t") + xlab("session") +
  geom_point(shape=1, color="gray50", size=2,
             position = position_jitter(width = 0.2, height = 0.0002))

p3 = ggplot(melt(M[,,3]), aes(x=as.factor(Var2), y=value)) + geom_violin() + 
  geom_boxplot(width=0.2) + ggtitle("post sup lat occ") + ylab("mean theta_t") + xlab("session") +
  geom_point(shape=1, color="gray50", size=2,
             position = position_jitter(width = 0.2, height = 0.0002))
