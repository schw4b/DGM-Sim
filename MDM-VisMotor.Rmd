...
PATH = file.path(PATH_HOME, 'Data', 'MDM') # path where figures will be stored
...
## Plot Connectivity strength to occipital fusiform (child node) for all 15 subjects and 5 sessions
```{r Plot connectivity strength, fig.height=15, fig.width=8, message=FALSE, warning=FALSE}
lim=c(-1, 1)
titles = c("rest","motor","visual","motor/visual i","motor/visual ii")
p = list()
c=1
for (s in 1:N) {
  for (r in 1:Ns) {
    if (s == 1) {
      p[[c]] = ggplot(melt(smt[,2:4,r,s]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) +
        geom_line() + theme_minimal() + ylim(lim) + theme(legend.position="none", axis.title.x=element_blank(),
                                                          axis.title.y=element_blank()) + ggtitle(titles[r])
      c=c+1
    } else{
      p[[c]] = ggplot(melt(smt[,2:4,r,s]), aes(x = Var1, y = value, group=Var2, color=as.factor(Var2))) +
        geom_line() + theme_minimal() + ylim(lim) + theme(legend.position="none", axis.title.x=element_blank(),
...
plot_grid(plotlist = p, ncol = Ns, nrow = N)
```

## Mean/SD of the theta_t for different runs
...
p[[i]] = ggplot(melt(M[,,i]), aes(x=as.factor(Var2), y=value)) + 
  geom_boxplot(width=0.4) + ggtitle(mylabels[pars][i]) + ylab("mean theta_t") + xlab("session") +
  geom_point(shape=1, color="gray70", size=2, position = position_jitter(width = 0.2, height = 0.0002))

p[[i+3]] = ggplot(melt(SD[,,i]), aes(x=as.factor(Var2), y=value)) + 
  geom_boxplot(width=0.4) + ggtitle(mylabels[pars][i]) + ylab("SD theta_t") + xlab("session") +
  geom_point(shape=1, color="gray70", size=2, position = position_jitter(width = 0.2, height = 0.0002))
}

plot_grid(plotlist = p, ncol = 3, nrow = 2)
```
