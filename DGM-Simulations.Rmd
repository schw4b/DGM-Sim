...
PATH_DATA = file.path(PATH, 'DGM-Sim', 'data') # path where time series data is
PATH_RES = file.path(PATH, 'DGM-Sim') # path where DGM data is 
```
...
ts.long = readMat(file.path(PATH_DATA,'Nn5_TR2_Noise01_HRF4_Mod1_Inj0_F1_60min.mat'))$gfy2s
```
...
subj=list()
for (s in 1:N) {
  subj[[s]] = read.subject(file.path(PATH_RES,'sim22'), sprintf("Id_%03d",s), Nn, e = 26)
}
dgm.sim22_e26=dgm.group(subj)

subj=list()
for (s in 1:N) {
  subj[[s]] = read.subject(file.path(PATH_RES,'Nn5_TR2_Noise01_HRF4_Mod1_Inj0_F1_60min'), sprintf("Id_%03d",s), Nn)
}
