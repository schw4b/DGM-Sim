...
gplotMat(atrue, title = "true network", hasColMap = F)
```
...
```{r, fig.height=7.8, fig.width=7.2}

s = 0.2 # spacing
pA=ggplot(melt(table.perf[1:3,c(1,5,6,7,9,11)]), aes(x=Var2, y=value, fill=Var1)) +
  ...
  ggtitle("dynamic") + coord_cartesian(ylim=c(0.4,1)) + scale_fill_brewer(palette="Set1")

pB=ggplot(melt(table.perf[1:3,c(2,8,10,12)]), aes(x=Var2, y=value, fill=Var1)) +
...
pMid  = plot_grid(pC1, pC2, pC3, pC4, pC5, pC6, pC7, pC8, nrow=2, ncol=4, rel_widths = c(1,1,1,1)) +
  theme(plot.margin = unit(c(s, 0, s, 0), "cm"))

pBott = plot_grid(pD1, pD2, pD3, pD4, pD5, pD6, pD7, pD8, nrow=2, ncol=4, rel_widths = c(1,1,1,1)) +
  theme(plot.margin = unit(c(s, 0, s, 0), "cm"))
