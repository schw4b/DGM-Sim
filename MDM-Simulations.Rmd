...
## Mean Sensitivity, specificity and accuracy for MDM and Patel's
apply(perf.mdm$sim1[,c(1,2,8)], 2, median)
apply(perf.pat$sim1[,c(1,2,8)], 2, median)

apply(perf.mdm$sim22[,c(1,2,8)], 2, median)
apply(perf.pat$sim22[,c(1,2,8)], 2, median)

```

## Plot: True network and correlation matrix 
```{r True network and correlation matrix, message=FALSE, warning=TRUE, fig.height=6.5, fig.width=6}
...
#p0 = ggplot(comput, aes(x=nodes, y=time))  + geom_line() + geom_point()
p0 = ggplot(data=comput, aes(x=nodes, y=time)) + geom_bar(stat="identity", fill="steelblue") +
...
top=plot_grid(p0, labels=c("A"), ncol = 2, nrow = 1, rel_widths = c(1, 0.2))
mid=plot_grid(p1, p2, labels=c("B", "C"), ncol = 2, nrow = 1, rel_widths = c(1, 0.4))
bot=plot_grid(p3, p4, labels=c("D", "E"), ncol = 2, nrow = 1, rel_widths = c(1, 1))

plot_grid(top, mid, bot, ncol=1, rel_heights = c(0.9, 0.95, 1))
...
#int=as.factor(c(rep(0,l), rep(1,l), rep(2,l), rep(3,l), rep(4,l), rep(5,l), rep(6,l)))
int=as.factor(c(rep("0 no",l), rep("1 0.4s",l), rep("2 0.8s",l), rep("3 1.1s",l), rep("4 1.4s",l), rep("5 1.6s",l), rep("6 1.8s",l)))
...
## Plot: MDM vs. Patel 
```{r Plot: MDM vs. Patel, message=FALSE, warning=TRUE, fig.height=5.5, fig.width=5.5}

p1 = gplotMat(stats.mdm.sim1$adj, 'MDM', '%', hasColMap = F)
p2 = gplotMat(stats.pat.sim1$adj, 'Patel', '%')
...
p3 = ggplot(d1, aes(x=gm, y=Percent, fill=NULL)) + geom_violin() + geom_boxplot(width=0.4) +
   geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.5, height = 0.1)) +
  ...
  scale_x_discrete(name ="Sensitivity  Specificity",
                   ...
                   labels=c("MDM", "MDM", "Pat", "Pat"))+ ggtitle("stationary")

p4 = gplotMat(stats.mdm.sim22$adj, 'MDM', '%', hasColMap = F)
p5 = gplotMat(stats.pat.sim22$adj, 'Patel', '%')
...
p6 = ggplot(d22, aes(x=gm, y=Percent, fill=NULL)) + geom_violin() + geom_boxplot(width=0.4) +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.5, height = 0.1)) +
  theme(legend.position="none", text = element_text(size=10), axis.text = element_text(size=9),
        panel.grid.major = element_line(colour = "gray80", linetype = "dotted"),
        panel.grid.major.x = element_blank()) +
  scale_x_discrete(name ="Sensitivity  Specificity",
                   breaks=c("MDM.Sens","MDM.Spec","Patel.Sens", "Patel.Spec"),
                   labels=c("MDM", "MDM", "Pat", "Pat")) + ggtitle("dynamic")
top=plot_grid(p4, p5, p6, p1, p2, p3, labels=c("A", "", "B", "C", "", "D"), ncol = 3, nrow = 2, rel_widths = c(0.65, 1, 0.7))

p7=gplotMat(rmna(stats.mdm.sim1$adj_fdr), 'MDM', '%', hasColMap = F, axisTextSize = 10)
p8=gplotMat(rmna(stats.pat.sim1$adj_fdr), 'Patel', '%', axisTextSize = 10)
p9=gplotMat(rmna(stats.mdm.sim22$adj_fdr), 'MDM', '%', hasColMap = F, axisTextSize = 10)
p10=gplotMat(rmna(stats.pat.sim22$adj_fdr), 'Patel', '%', hasColMap = F, axisTextSize = 10)

bot=plot_grid(p9, p10, p7, p8, labels=c("E", "", "F", ""), ncol = 4, nrow = 1, rel_widths = c(0.58, 0.58, 0.58, 1))

plot_grid(top, bot, ncol=1, rel_heights = c(1, 0.42))
...
d1=data.frame(df=c(mdm.sim1$df_),  node=node)
d2=data.frame(df=c(mdm.sim22$df_), node=node)
d3=data.frame(df=c(mdm.int0$df_),  node=node)
d4=data.frame(df=c(mdm.int1$df_),  node=node)
d5=data.frame(df=c(mdm.int2$df_),  node=node)
d6=data.frame(df=c(mdm.int3$df_),  node=node)
d7=data.frame(df=c(mdm.int4$df_),  node=node)
d8=data.frame(df=c(mdm.int5$df_),  node=node)
d9=data.frame(df=c(mdm.int6$df_),  node=node)

p1 = ggplot(d1, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Sim1") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.07))
  
p2 = ggplot(d2, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Sim22") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.07))

p3 = ggplot(d3, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int0") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p4 = ggplot(d4, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int1") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p5 = ggplot(d5, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int2") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p6 = ggplot(d6, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int3") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p7 = ggplot(d7, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int4") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p8 = ggplot(d8, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int5") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

p9 = ggplot(d9, aes(x=node, y=df)) + geom_violin() + geom_boxplot(width=0.4) + ggtitle("Int6") +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.05))

plot_grid(p1, p2, p3, p4, p5, p6, p7, p8, p9, ncol = 3, nrow = 3)
...
```{r Plot: MDM sensitivity and specificity for the 7 HRF datasets, fig.height=4, fig.width=3, message=FALSE}
...
p1 = ggplot(myd, aes(x=Method, y=Percent)) +   geom_violin() +  geom_boxplot(width=0.2) +
  ...
  theme(panel.grid.major = element_line(colour = "gray70", linetype = "dotted"),
...
p2 = ggplot(myd, aes(x=Method, y=Percent)) +   geom_violin() +  geom_boxplot(width=0.4) +
  geom_point(shape=1, color="gray70", size=0.5, position = position_jitter(width = 0.4, height = 0.07)) +
  geom_line(aes(x=as.numeric(Method), y=Med), size=0.3) + 
  coord_cartesian(ylim = c(0, 1)) +
  ggtitle('Specificity') + 
...
# Sensitivity
c(median(perf.mdm$int0[,1]), median(perf.mdm$int1[,1]), median(perf.mdm$int2[,1]), median(perf.mdm$int3[,1]),
  median(perf.mdm$int4[,1]), median(perf.mdm$int5[,1]), median(perf.mdm$int6[,1]), median(perf.mdm$null[,1]))

# Specificity
c(median(perf.mdm$int0[,2]), median(perf.mdm$int1[,2]), median(perf.mdm$int2[,2]), median(perf.mdm$int3[,2]),
