...
TR=0.72
labels = read.table(file.path(PATH, 'Net10Labels.txt'), header = T)
...
# 
# f=file(file.path(PATH,"net10.RData"))
# save(mdm.net10.r1, mdm.net10.r2, mdm.net10.r3, mdm.net10.r4, file = f)
# close(f)
load(file.path(PATH, "net10.RData"))
```
...
# save(ts, file = f)
# close(f)
load(file.path(PATH, "ts10.RData"))
```

## Correlation Plots
```{r Correlation Plots, fig.height=4.5, fig.width=6.5}
LIM=c(-0.6,0.6)

p1 = gplotMat(rmdiag(corTs(ts[,,1,])), title=sprintf("Run 1 (N=%d)", N), lim=LIM,
              colMapLabel=expression("Pearson\'s"~italic(r)), gradient = c("blue", "white", "red")) + xlab("Node") + ylab("Node")
p2 = gplotMat(rmdiag(corTs(ts[,,2,])), title=sprintf("Run 2 (N=%d)", N), lim=LIM,
              colMapLabel=expression("Pearson\'s"~italic(r)), gradient = c("blue", "white", "red")) + xlab("Node") + ylab("Node")
p3 = gplotMat(rmdiag(corTs(ts[,,3,])), title=sprintf("Run 3 (N=%d)", N), lim=LIM,
              colMapLabel=expression("Pearson\'s"~italic(r)), gradient = c("blue", "white", "red")) + xlab("Node") + ylab("Node")
p4 = gplotMat(rmdiag(corTs(ts[,,4,])), title=sprintf("Run 4 (N=%d)", N), lim=LIM,
              colMapLabel=expression("Pearson\'s"~italic(r)), gradient = c("blue", "white", "red")) + xlab("Node") + ylab("Node")
...
stats.r1 = binom.nettest(mdm.net10.r1$am)
stats.r2 = binom.nettest(mdm.net10.r2$am)
stats.r3 = binom.nettest(mdm.net10.r3$am)
stats.r4 = binom.nettest(mdm.net10.r4$am)
...
```{r Networks consistency across runs, fig.height=10, fig.width=6.2}
...
p3 = gplotMat(stats.m4$adj, title = "edge in 4/4 runs", nodeLabels=labels$Label,
              axisTextSize=8, xAngle=90, lim = c(0, 0.5))
p4 = gplotMat(rmna(stats.m4$adj_fdr), title = "binom test & FDR",
              nodeLabels=labels$Label, axisTextSize=8, xAngle=90, lim = c(0,0.5))
...
plot_grid(p1, p2, p3, p4, p5, p6, ncol = 2, nrow = 4, rel_heights = c(1,1,1,1))
```
